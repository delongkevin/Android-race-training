...
public class GetRaceListingsViewModel extends AndroidViewModel {
    private MutableLiveData<ApiResponse> apiResponseMutableLiveData;
    private final CompositeDisposable mDisposables = new CompositeDisposable();

    public GetRaceListingsViewModel(@NonNull Application application) {
        super(application);
    }
    ...
    public void getRaceListing(String token, int page) {
        mDisposables.add(NetworkRepository.getInstance().getRaceListing(token, page)
    ...
    public void getRaceListing(String token, int page, int year, int month) {
        mDisposables.add(NetworkRepository.getInstance().getRaceListing(token, page, year, month)
                .subscribeOn(Schedulers.io())
                .timeout(180, TimeUnit.SECONDS)
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(
